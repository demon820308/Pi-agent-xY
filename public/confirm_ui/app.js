/* PPT Master - Eight Confirmations UI (Next.js native hosted)
 ...
 * /confirm_ui/catalogs.json with the AI's recommendation marked. Open/generative
...
(function () {
    ...
    function qs() { return SESSION_ID ? "?sessionId=" + encodeURIComponent(SESSION_ID) : ""; }
    ...
    var STATE = {};
    var rebuildColorSection = function () {};

    function getRecommendedColor() {
        var cc = (REC && REC.color && REC.color.candidates) || [];
        var csel = (REC && REC.color && REC.color.selected) || 0;
        var c0 = cc[Math.min(csel, Math.max(cc.length - 1, 0))] || {};
        return { name: c0.name || "", palette: Object.assign({}, normPalette(c0)) };
    }

    function getStyleBgType(styleId) {
        var darkStyles = ["glassmorphism", "dark-tech", "blueprint", "photo-editorial", "chalkboard", "pixel-art"];
        var paperStyles = ["editorial", "zine", "vintage-poster", "paper-cut", "sketch-notes", "ink-notes", "ink-wash"];
        if (darkStyles.indexOf(styleId) >= 0) return "dark";
        if (paperStyles.indexOf(styleId) >= 0) return "paper";
        return "light";
    }
    ...
    function rgbToHex(r, g, b) {
        var rHex = Math.max(0, Math.min(255, Math.round(r))).toString(16);
        var gHex = Math.max(0, Math.min(255, Math.round(g))).toString(16);
        var bHex = Math.max(0, Math.min(255, Math.round(b))).toString(16);
    ...
    function ensureReadableOnDark(hex) {
        ...
        if (lightness < 100) {
            var factor = 1.8;
            return rgbToHex(
                Math.min(255, rgb.r * factor + 50),
                Math.min(255, rgb.g * factor + 50),
                Math.min(255, rgb.b * factor + 50)
            );
        }
        return hex;
    }
    ...
    function adaptPalette(pal, bgType) {
        var originalPrimary = pal.primary || "#1a3a6b";
        var originalAccent = pal.accent || originalPrimary;
        if (bgType === "dark") {
            var priRgb = hexToRgb(originalPrimary);
            var tintBg = rgbToHex(priRgb.r * 0.05 + 5, priRgb.g * 0.05 + 7, priRgb.b * 0.08 + 12);
            return {
                background: tintBg,
                secondary_bg: rgbToHex(priRgb.r * 0.08 + 15, priRgb.g * 0.08 + 20, priRgb.b * 0.12 + 30),
                primary: ensureReadableOnDark(originalPrimary),
                accent: ensureReadableOnDark(originalAccent),
                secondary_accent: "#707e94",
                body_text: "#e2e8f0"
            };
            ...
            return {
                background: "#fbfaf6",
                secondary_bg: "#f3f0e6",
                primary: ensureReadableOnLight(originalPrimary),
                accent: ensureReadableOnLight(originalAccent),
                secondary_accent: "#7c8491",
                body_text: "#1c1d1f"
            };
        } else {
            return {
    ...
    function updateColorSelectionForStyle(styleId) {
        var recColor = getRecommendedColor();
        var bgType = getStyleBgType(styleId);
        var adapted = adaptPalette(recColor.palette, bgType);
        
        var isRecStyle = (styleId === recId("visual_style"));
        var displayName = isRecStyle ? (recColor.name || t("recommended")) : ((recColor.name || t("recommended")) + " (" + (LANG === "zh" ? "智能适配" : "Adaptive") + ")");
    ...
    function enumField(parent, list, recommendedId, getVal, setVal, opts2) {
        ...
        var allowCustom = opts2.allowCustom === true;
    ...
    function renderStyle(host) {
        ...
        enumField(sub2, CAT.visual_styles, recOrFirst("visual_style", CAT.visual_styles),
            function () { return STATE.visual_style; },
            function (v) {
                var oldStyle = STATE.visual_style;
                STATE.visual_style = v;
                if (oldStyle !== v) {
                    updateColorSelectionForStyle(v);
                    rebuildColorSection();
                }
            }, { allowCustom: true });
    ...
    function renderColor(host) {
        var recColor = getRecommendedColor();
        var bgType = getStyleBgType(STATE.visual_style);
        var adaptedPalette = adaptPalette(recColor.palette, bgType);
        
        var isRecStyle = (STATE.visual_style === recId("visual_style"));
        var displayName = isRecStyle ? (recColor.name || t("recommended")) : ((recColor.name || t("recommended")) + " (" + (LANG === "zh" ? "智能适配" : "Adaptive") + ")");
        
        var adaptedCandidate = {
            name: displayName,
            palette: adaptedPalette,
            note: LANG === "zh" ? "根据 PPT 主题进行明度与对比度优化" : "Luminance and contrast optimized for PPT theme"
        };
        
        var cands = [adaptedCandidate];
        
        var foundStyle = null;
        (CAT.visual_styles || []).forEach(function (group) {
            (group.items || []).forEach(function (item) {
                if (item.id === STATE.visual_style) foundStyle = item;
            });
        });
        
        var presets = (foundStyle && foundStyle.color_presets) || [];
    ...
    function renderAll() {
        ...
        var colorContainer = el("div", "color-section-container");
        styleGroup.appendChild(colorContainer);
        rebuildColorSection = function () {
            colorContainer.innerHTML = "";
            renderColor(colorContainer);
        };
        rebuildColorSection();
    ...
    function confirm() {
        ...
        fetch("/api/confirm" + qs(), {
            ...
            try {
                window.parent.postMessage({ type: "PPT_CONFIRM_DONE" }, "*");
            } catch (e) {}
            fetch("/api/shutdown" + qs(), {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    function loadCatalogs() {
        return fetch("/api/catalogs" + qs())
            .then(function (r) { if (r.ok) return r.json(); throw new Error("no api"); })
            .catch(function () { return fetch("/confirm_ui/catalogs.json").then(function (r) { return r.json(); }); });
    }
    ...
    function boot() {
        ...
        toggleBtn.addEventListener("click", function () {
            ...
            applyStaticTranslations();
            refreshLangToggle(toggleBtn);
            if (REC && CAT) renderAll();
        });
        document.getElementById("btn-confirm").addEventListener("click", confirm);

        Promise.all([
            loadCatalogs(),
            fetch("/api/recommendations" + qs()).then(function (r) { if (!r.ok) throw new Error("load failed"); return r.json(); })
        ]).then(function (res) {
            CAT = res[0];
            REC = res[1];
